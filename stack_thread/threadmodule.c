...
local_new(PyTypeObject *type, PyObject *args, PyObject *kw)
{
        ...
        self->key = PyString_FromFormat("stack_thread.local.%p", self);
        if (self->key == NULL) 
                goto err;
...
initstack_thread(void)
{
	...
	m = Py_InitModule3("stack_thread", thread_methods, thread_doc);

	/* Add a symbolic constant */
	d = PyModule_GetDict(m);
	ThreadError = PyErr_NewException("stack_thread.error", NULL, NULL);
