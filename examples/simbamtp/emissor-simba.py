...
simbahost="corellia.dsic.upv.es"
...
class emissor(Agent.Agent):
    ...
    class BehaviourDefecte(Behaviour.Behaviour):
        ...
        def done(self): 
            ...
            if self.myAgent.nenviats == self.myAgent.nmsg: return True
            else: return False
	...
	#print "EMISOR: "+ str(self.getQueue())
        ...
        if self.multi:
            ...
            self.msg.addReceiver(AID.aid("receptor0@"+host,["simba://"+simbahost]))
        else:
            self.msg.addReceiver(AID.aid("receptor%i@"%(self.nagent)+host,["simba://"+simbahost]))
